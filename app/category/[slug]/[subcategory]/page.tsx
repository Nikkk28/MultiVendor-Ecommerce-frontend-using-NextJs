...
export default function SubcategoryPage({ params }: { params: { slug: string; subcategory: string } }) {
  ...
  // Fetch category, subcategory and products data
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Find subcategory
        const foundSubcategory = categoryData.subcategories?.find((s: any) => s.slug === params.subcategory)
        if (!foundSubcategory) {
    ...
    return (
      <div className="container mx-auto px-4 py-12">
        <Alert variant="destructive">
          <AlertDescription>{error || "Category or subcategory not found"}</AlertDescription>
        </Alert>
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-2">Browse our collection of {subcategory.name.toLowerCase()} products</p>
      </div>
